...
namespace Contractor
{
    public partial class users : Form
    {
        ...
        private void AddUsrBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                bool active = activeCheckBox.Checked,
                 notify = notifacationsCheckBox.Checked,
                 projects = projectsCheckBox.Checked,
                 payment = paymentCheckBox.Checked,
                 client = clientsCheckBox.Checked,
                 statis = statisticsCheckBox.Checked,
                 sit = sittingCheckBox.Checked,
                 user = usersCheckBox.Checked,
                 payments_type = paymentstypeCheckBox.Checked;

                int id = int.Parse(this.usersTableAdapter.InsertReturnId(name, username, password, printer, nationalID, phone, active).ToString());
                this.users_privilageTableAdapter.Insert(id, notify,client,user,projects,payment,payments_type,sit,statis);
                ...
                if (usernameTextBox.Text != "" && usernameTextBox.Text != string.Empty && usernameTextBox.Text != null)
                {
                    ...
                    if (product != null)
                    {
                     ...
                     payments_type = paymentstypeCheckBox.Checked;
                        ...
                        this.users_privilageTableAdapter.Update(id, notify, client, user, projects, payment, payments_type, sit, statis);
        ...
        private void SearchTxt_TextChanged(object sender, EventArgs e)
        {
            try
            {
                this.usersTableAdapter.Search(this.contractorDataSet.users, Search_textBox.Text);
            }
            catch (Exception errortext)
            {
                LoginInfo.log(" حدث خطأ فى " + this.Text + " " + this.Name.ToString() + " " + errortext.ToString());
        ...
        private void userViewDataGridView_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int id = int.Parse(product.Cells[0].Value.ToString());
                    this.users_privilageTableAdapter.FillBy(this.contractorDataSet.users_privilage, id);
                }
            }
            catch (Exception errortext)
            {
