...
using Contractor.ContractorDataSetTableAdapters;
            ...
            try
            {
                ...
                gBgwDownload.DoWork += (object ss, System.ComponentModel.DoWorkEventArgs ee) =>
                {
                    try
                    {
                        ...
                        SqlConnection cn = new SqlConnection(Properties.Settings.Default.ContractorConnectionString);
                        SqlCommand cmd = new SqlCommand("use Contractor DECLARE @path nvarchar(max) select @path=backup_path from sittings BACKUP DATABASE Contractor To DISK=@path ;", cn);
